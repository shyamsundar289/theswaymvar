...
const crewData: CrewMember[] = Array.from({ length: 18 }).map((_, i) => {
  ...
  return {
    ...
    description:
      "Drawn to movement, music and the quiet moments between celebrations, they turn wedding days into films that feel like memories rather than productions.",
    image: assets.crew.getMemberPhoto(num),
...
const imageVariants: Record<CrewMember["layout"], Variants> = {
  "editorial-split": {
    ...
    exit: (dir: SplitDirection) => ({
      clipPath: dir > 0 ? "inset(0 0 100% 0)" : "inset(100% 0 0 0)",
      scale: 0.95,
      filter: "blur(10px)",
      transition: { duration: transitionDuration * 0.8, ease: transitionEase },
    }),
  },
  "cinema-center": {
...
const textVariants: Variants = {
  ...
  center: {
    ...
    opacity: 1,
    filter: "blur(0px)",
    transition: { duration: 1, ease: transitionEase, delay: 0.2, staggerChildren: 0.05 },
  },
  exit: (dir: SplitDirection) => ({
    y: dir > 0 ? -40 : 40,
    opacity: 0,
...
const CrewMemberSlide = ({ member, direction }: { member: CrewMember; direction: number }) => {
    ...
    return (
      <div className="absolute inset-0 w-full h-full flex flex-col md:flex-row items-center justify-between px-6 py-24 md:p-24 gap-8 md:gap-12 bg-background text-foreground">
        <motion.div
          ...
          className="w-full md:w-5/12 flex flex-col justify-center z-10"
        >
          <p className="font-display text-muted-foreground text-[clamp(4rem,10vw,8rem)] tracking-tighter mb-2 md:mb-4">
            {member.number}
          </p>
          ...
          <p className="text-xs text-muted-foreground mb-4 md:mb-6 uppercase tracking-[0.2em]">
            {member.role}
          </p>
  ...
  if (layout === "cinema-center") {
    return (
      <div className="absolute inset-0 w-full h-full flex flex-col items-center justify-center p-6 md:p-12 bg-background text-foreground">
        ...
        <motion.div
          variants={textVariants}
          className="relative z-10 flex flex-col items-center text-center mt-auto pb-12 md:pb-24 w-full max-w-3xl px-4"
        >
          <p className="font-display text-muted-foreground text-2xl md:text-3xl mb-4">
            {member.number} / 18
          </p>
          ...
          <p className="text-xs md:text-sm text-foreground uppercase tracking-[0.2em] mb-4">
            {member.role}
          </p>
  ...
  if (layout === "offset-minimal") {
    return (
      <div className="absolute inset-0 w-full h-full flex flex-col md:flex-row items-center p-6 md:p-24 bg-background text-foreground">
        ...
        <motion.div
          variants={textVariants}
          className="w-full md:w-1/2 flex flex-col text-right md:pl-12 mt-8 md:mt-0 z-10"
        >
          <h2 className="font-display font-light text-[clamp(1.75rem,8vw,4.5rem)] mb-4 uppercase tracking-tighter ml-auto max-w-[12ch]">
            <StaggeredText text={member.name} />
          </h2>
          <p className="text-xs text-muted-foreground mb-6 uppercase tracking-[0.3em]">
            {member.role}
          </p>
          <div className="w-full h-px bg-border mb-6" />
          <p className="text-sm md:text-base text-foreground ml-auto max-w-sm">
            {member.description}
          </p>
          <p className="font-display text-muted-foreground text-[clamp(4.5rem,10vw,8rem)] mt-8 md:mt-12 transform md:translate-x-4">
            {member.number}
          </p>
        </motion.div>
      </div>
  ...
  if (layout === "landscape-overlap") {
    return (
      <div className="absolute inset-0 w-full h-full flex items-center justify-center p-4 md:p-16 bg-background text-foreground">
        ...
        <motion.div
          variants={textVariants}
          className="absolute bottom-8 left-4 right-4 md:right-auto md:bottom-24 md:left-24 z-10 bg-background/95 backdrop-blur-md p-6 md:p-10 border border-border max-w-lg md:max-w-xl shadow-2xl"
        >
          <div className="flex items-center gap-4 md:gap-6 mb-4 md:mb-6">
            <span className="font-display text-3xl md:text-4xl text-muted-foreground">
              {member.number}
            </span>
  ...
  return (
    <div className="absolute inset-0 w-full h-full flex items-center justify-center bg-background text-foreground overflow-hidden">
      <motion.div
        variants={imageVariants["vertical-hero"]}
        className="absolute right-0 top-0 bottom-0 w-full md:w-1/2 h-full opacity-30 md:opacity-100"
      >
        <img
          src={member.image}
          alt={member.name}
          className="w-full h-full object-cover object-center grayscale-[30%] hover:grayscale-0 transition-all duration-1000"
        />
        <div className="absolute inset-0 bg-background/60 md:hidden" />
      </motion.div>
      <motion.div
        variants={textVariants}
        className="relative z-10 w-full px-6 md:px-24 flex flex-col justify-center"
      >
        <div className="flex flex-col max-w-xl">
          <p className="font-display text-muted-foreground text-[clamp(2rem,4vw,4rem)] mb-4">
            {member.number} / 18
          </p>
...
function CrewPage() {
  ...
  return (
    <div className="bg-background text-foreground min-h-[100svh] selection:bg-foreground selection:text-background font-sans">
      ...
      >
        ...
        <div className="absolute inset-0 w-full h-full">
          ...
          <div
            ...
            style={{
              background:
                "radial-gradient(ellipse at 50% 42%, rgba(0,0,0,0) 35%, rgba(0,0,0,0.6) 100%)",
            }}
          />
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(180deg, rgba(201,168,118,0.35) 0%, rgba(20,16,12,0) 45%, rgba(201,168,118,0.3) 100%)",
            }}
          />
        </div>
        ...
        >
          <path
            d="M0,0 L1440,0 L1440,100 Q720,195 0,100 Z"
            fill="url(#archGradientTop)"
            opacity="0.6"
          />
          ...
          <path
            d="M0,68 Q720,150 1440,68"
            fill="none"
            stroke="var(--accent, #C9A876)"
            strokeWidth="1.5"
            opacity="0.55"
          />
        </svg>
        ...
        >
          <path
            d="M0,260 L1440,260 L1440,160 Q720,65 0,160 Z"
            fill="url(#archGradientBottom)"
            opacity="0.6"
          />
          <path d="M0,260 L1440,260 L1440,192 Q720,110 0,192 Z" fill="url(#archGradientBottom)" />
          <path
            d="M0,192 Q720,110 1440,192"
            fill="none"
            stroke="var(--accent, #C9A876)"
            strokeWidth="1.5"
            opacity="0.55"
        ...
        >
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(180deg, rgba(10,8,6,0.8) 0%, rgba(10,8,6,0.5) 55%, rgba(10,8,6,0) 100%)",
            }}
          />
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(90deg, transparent 0%, var(--accent, #C9A876) 50%, transparent 100%)",
            }}
          />
          ...
          <div className="relative flex items-center justify-between px-6 md:px-12 py-5 md:py-7">
            ...
            <Link to="/" className="group flex flex-col items-center shrink-0 px-4 md:px-8">
              <span
                ...
                style={{
                  border: "1px solid var(--accent, #C9A876)",
        ...
        >
          ...
          <motion.span
            className="h-10 w-px"
            style={{
              background: "linear-gradient(180deg, var(--accent, #C9A876) 0%, transparent 100%)",
            }}
      ...
      <section
        ref={btsVideoRef}
        className="h-[calc(100svh-var(--header-height))] bg-background relative z-20 flex items-center justify-center p-4 md:p-10 lg:p-14"
      >
        <div className="relative w-full h-full max-w-[1600px] overflow-hidden rounded-[4px]">
          ...
          <div className="w-full h-full relative bg-foreground overflow-hidden">
            <video
              src={assets.videos.background.videoseen}
              autoPlay
              loop
              muted
              playsInline
      ...
      <section className="py-32 md:py-48 bg-white border-t border-border flex flex-col items-center justify-center text-center px-4 relative z-20">
        ...
        <p className="max-w-2xl text-foreground/80 text-sm md:text-base leading-relaxed mb-12">
          We are a team of passionate visual storytellers operating globally. We believe in
          authenticity, cinematic aesthetics, and preserving the raw emotion of your most important
          celebrations.
        </p>
        <Link
          to="/about"
          className="inline-block border border-foreground text-foreground px-8 py-4 text-xs uppercase tracking-[0.2em] hover:bg-foreground hover:text-background transition-colors duration-500 cursor-pointer"
        >
          Discover More
        </Link>
      </section>
