...
export function FilmChapter({ chapter, index }: FilmChapterProps) {
  ...
  return (
    <div
      className={`grid gap-8 md:grid-cols-2 md:items-center md:gap-16 lg:gap-24 ${isEven ? "" : "md:flex-row-reverse"}`}
    >
      ...
      <div className={`order-1 md:order-none ${isEven ? "md:order-2" : "md:order-1"}`}>
        <Reveal>
          <div className="flex items-end gap-6">
            ...
            <span className="label-xs mb-2 text-bronze">{chapter.timestamp}</span>
          </div>
