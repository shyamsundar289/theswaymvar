...
export function CinematicVideoCard({ film, isActive, onActivate }: CinematicVideoCardProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute inset-0 bg-black/30 transition-opacity duration-500 pointer-events-none ${isPlaying && !showControls ? "opacity-0" : "opacity-100"}`}
      >
        {/* Top Text */}
        <div className="absolute top-6 left-6 right-6 text-white drop-shadow-md">
          <p className="text-[10px] md:text-[11px] tracking-[3px] uppercase font-sans mb-1 opacity-80">
            {film.category}
          </p>
      ...
      >
        {/* Timeline */}
        <div
          className="w-full mb-3 group/timeline cursor-pointer flex items-center"
