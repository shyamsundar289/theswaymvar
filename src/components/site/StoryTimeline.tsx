...
function TimelineItem({ 
  ...
  // 4 items to animate (index 1 to 4) + 1 buffer segment = 5 segments
  const isFirst = index === 0;
  const animationIndex = index - 1; // Shifts index 1->0, 2->1, etc.
  const segmentSize = 1 / 5;
  const start = animationIndex * segmentSize;
  ...
  const contentBlock = (
    <div className={`flex flex-col w-full text-left md:text-${!isRightSide ? 'right' : 'left'} items-start md:items-${!isRightSide ? 'end' : 'start'}`}>
      <motion.span 
...
export function StoryTimeline() {
  ...
  return (
    <section 
      ref={containerRef}
      className="bg-background text-foreground font-sans relative mt-12 md:mt-24 h-[600vh] mb-[20vh] md:mb-[30vh]"
    >
      {/* The sticky viewport that locks the layout in place. Height adjusted to 100svh for perfect fit. */}
      <div className="sticky top-0 h-[100svh] min-h-[100svh] w-full flex items-center pt-16 pb-8 md:pt-16 md:pb-8 box-border">
        
        <div className="max-w-[1400px] mx-auto flex flex-col lg:flex-row w-full h-full items-center justify-center pt-8 sm:pt-12 md:pt-0">
          
          {/* LEFT: Static Text Section */}
          <div className="relative w-full lg:w-[40%] px-4 sm:px-8 lg:px-16 flex flex-col justify-center bg-background z-20 shrink-0 text-center lg:text-left mb-6 lg:mb-0 lg:max-h-full pt-4 md:pt-8">
            
            {/* Elegant Background Arc (Desktop only) */}
            <div className="hidden lg:block absolute -left-8 top-1/2 -translate-y-1/2 w-[110%] aspect-square border-l-[1px] border-t-[1px] border-b-[1px] border-[#d1cbbd]/30 rounded-l-full pointer-events-none z-0"></div>
            ...
            <div className="relative z-10 flex flex-col items-center lg:items-start">
              ...
              {/* Heading */}
              <h3 className="flex flex-col items-center lg:items-start text-foreground mb-4 text-center lg:text-left w-full">
                <span className="text-[10px] sm:text-[12px] md:text-[14px] lg:text-[16px] tracking-[0.3em] uppercase text-[#8b867c] mb-1">
                  Where Our
                </span>
                <span className="text-[26px] sm:text-[32px] md:text-[40px] lg:text-[48px] leading-[1.1] text-[#2d2c2a] opacity-90" style={{ fontFamily: "'Pinyon Script', cursive" }}>
                  Forever Began
                </span>
              </h3>
              ...
              {/* Paragraph */}
              <p className="text-xs sm:text-sm md:text-base lg:text-lg text-muted-foreground/80 max-w-md mx-auto lg:mx-0 leading-relaxed text-center lg:text-left w-full">
              ...
              <div className="mt-8 flex justify-center lg:justify-start w-full">
                <span className="text-[32px] sm:text-[40px] md:text-[48px] text-[#D0A17C]" style={{ fontFamily: "'Pinyon Script', cursive" }}>
                  Ritesh & Genelia
                </span>
              </div>
            </div>
          ...
          <div className="w-full lg:w-[60%] relative flex-1 flex flex-col justify-center mt-8 lg:mt-0 px-4">
            ...
            <div className="relative z-20 w-full flex flex-col justify-center items-center h-full">
              ...
              <div className="absolute left-1/2 top-[35px] sm:top-[45px] md:top-[64px] lg:top-[85px] xl:top-[100px] bottom-[35px] sm:bottom-[45px] md:bottom-[64px] lg:bottom-[85px] xl:bottom-[100px] w-[2px] border-l-[2px] border-dashed border-[#d1cbbd]/40 -translate-x-1/2 z-0"></div>

              {/* Animated Solid Cream Progress Line (follows overall scroll) */}
              <motion.div
                className="absolute left-1/2 top-[35px] sm:top-[45px] md:top-[64px] lg:top-[85px] xl:top-[100px] bottom-[35px] sm:bottom-[45px] md:bottom-[64px] lg:bottom-[85px] xl:bottom-[100px] w-[2px] bg-[#d1cbbd] -translate-x-1/2 origin-top z-10"
                style={{ scaleY: scrollYProgress }}
              />

              {/* Render Timeline Items */}
              <div className="w-full flex flex-col justify-center relative z-20 py-4">
